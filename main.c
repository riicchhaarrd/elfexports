...
int read_binary_file(const char *filename, u8 **buf, size_t *fs)
{
...
void *elf_get_section(elfx86_t *hdr, int section)
{
	elfx86_shdr_t *shdr = ELF_SECTION(hdr, section);
	return (void*)( ((u32)hdr) + shdr->sh_offset );
}

int elf_find_section_by_name(elfx86_t *hdr, const char *name)
{
